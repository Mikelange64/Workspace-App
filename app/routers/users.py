...
from app.schemas.users import UserCreate, UserPublic, UserPrivate, UserUpdate, ChangePassword, Token
...
def create_user(user: UserCreate, db: Annotated[Session, Depends(get_db)]):
    ...
    username_exists = db.execute(
        select(User)
        .where(func.lower(User.username) == user.username)
    ).scalars().first()
...
def get_user_workspaces(current_user: CurrentUser, db: Annotated[Session, Depends(get_db)]):
    workspace = db.execute(
        select(Workspace)
        .where(Workspace.admin_id  == current_user.id)
    ).scalars().all()
    return workspace
...
def update_user(
    ...
    if user_data.username is not None and user_data.username.lower() != current_user.username.lower():
...
def change_password(
    ...
    if verify_password(password_data.new_password, current_user.password_hash):
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="New password must be different to the old password")
